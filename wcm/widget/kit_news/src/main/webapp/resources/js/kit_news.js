var KitIntranetNews = SuperWidget
	.extend({
		...
		serviceGetNews: function() {
			...
			DatasetFactory.getDataset('kit_news', null, null, orderByConstraint, callback);
		},

		prepareNews: function(news) {
			console.log("prepareNews",news);
		...
		loadNews: function() {
			...
			if (this.NEWS && 
				this.NEWS.values?.length - this.OFFSET > 1) {
			...
			if (this.NEWS && 
				$('[data-show-news-detail]', this.DOM).length === this.NEWS.values.length) {
